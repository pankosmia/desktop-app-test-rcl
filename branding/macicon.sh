...
# 1. Create mac_icon.svg or mac_icon.png source file ≥ 1024px square, and place it in the `source` subdirectory, one level down from `branding`.
# 2. Run this script in a terminal by entering: `./macicon.sh [inkscape|imagemagick|magick] [svg|png]` from the `branding` directory.
...
# All icon_*.png files are building blocks for icon.icns (macOS icon format)
...
PNG_EXISTS=0
SVG_EXISTS=0
[ -f "source/mac_icon.png" ] && PNG_EXISTS=1
[ -f "source/mac_icon.svg" ] && SVG_EXISTS=1
...
elif [ "$CONVERSION_TOOL" = "magick" ]; then
