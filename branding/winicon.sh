...
# 1. Create win_icon_1024x1024.png and place it in the `source` subdirectory, one level down from `branding.
# 2. Run this script in a terminal by entering: `./winicon.sh` from the `branding` directory.
# Note that re-running this script over-writes files it just created (or any other files of the same names).
